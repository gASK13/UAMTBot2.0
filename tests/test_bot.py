...
class TestBotHandler(unittest.TestCase):
    ...
    def test_slap_w_nick(self):
        self.bot.handle_command(2, {'name': 'slap', 'options': [{'value': 'test test'}]}, {'nick': 'NICK'}, 'token')
        self.bot.poster.patch.assert_called_with(url='https://discord.com/api/v9/webhooks/N/A/token/messages/@original',
                                                 json={
    ...
    def test_slap_no_nick(self):
        self.bot.handle_command(2, {'name': 'slap', 'options': [{'value': 'test test'}]},
    ...
    def test_slap_no_user(self):
        self.bot.handle_command(2, {'name': 'slap', 'options': [{'value': 'test test'}]}, {}, 'token')
        self.bot.poster.patch.assert_called_with(url='https://discord.com/api/v9/webhooks/N/A/token/messages/@original',
                                                 json={
